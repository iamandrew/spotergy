class SpotifyPlayer {
  ...
  _onNewAccessToken() {
    if (this.accessToken === '') {
      ...
      this.fetchUser().then(user => {
        ...
        this.loopInterval = setInterval(loop.bind(this), 2000);
        loop();
      });
    }
  }
  ...
  getPlaylist(userid, playlistid) {
    return this.fetchGeneric('https://api.spotify.com/v1/users/' +  encodeURIComponent(userid) + '/playlists/' + playlistid).then(data => data.json());
  }

  getUser(userid) {
    return this.fetchGeneric('https://api.spotify.com/v1/users/' +  encodeURIComponent(userid)).then(data => data.json());
  }
}